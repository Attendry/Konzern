...
export class AuditDocumentationTool implements AgentTool {
  ...
  async execute(
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          stats: {
            ...
            checksPassed: data.plausibilityChecks.filter(c => c.result === 'PASS').length,
            icCount: data.icReconciliations.length,
            icOpen: data.icReconciliations.filter(ic => ic.status === 'open').length,
          },
        },
      ...
      return this.createErrorResult(`Generierung fehlgeschlagen: ${error.message}`);
    }
  }
  ...
  private async fetchDocumentationData(
    ...
    const [fsResult, entriesResult, checksResult, icResult, companiesResult] = await Promise.all([
      client
  ...
  private async generateDocumentation(
    ...
    lines.push(`**Stichtag:** ${fs.reporting_date || fs.fiscal_year + '-12-31'}`);
    ...
    if (section === 'all' || section === 'scope') {
      ...
      for (const company of data.companies.slice(0, 20)) {
        lines.push(`| ${company.name} | ${company.legal_form || '-'} | ${company.ownership_percentage || 100}% |`);
      }
    ...
    if (section === 'all' || section === 'entries') {
      ...
      for (const [type, entries] of Object.entries(byType)) {
        ...
        const total = entries.reduce((sum, e) => sum + Math.abs(e.amount || 0), 0);
        lines.push(`**Gesamtvolumen:** €${total.toLocaleString('de-DE', { minimumFractionDigits: 2 })}`);
        lines.push('');
        ...
        for (const entry of entries.slice(0, 5)) {
          lines.push(`| ${entry.description || entry.adjustment_type} | €${(entry.amount || 0).toLocaleString('de-DE')} | ${entry.created_at?.substring(0, 10) || '-'} |`);
        }
        if (entries.length > 5) {
    ...
    if (section === 'all' || section === 'ic') {
      ...
      const resolved = data.icReconciliations.filter(ic => ic.status === 'resolved');
      ...
      if (open.length > 0) {
        ...
        for (const ic of open.slice(0, 10)) {
          lines.push(`| ${ic.company_a?.name || '-'} | ${ic.company_b?.name || '-'} | €${Math.abs(ic.difference_amount || 0).toLocaleString('de-DE')} | ${ic.status} |`);
        }
        lines.push('');
      }
    }
    ...
    if (section === 'all' || section === 'checks') {
      ...
      const warning = data.plausibilityChecks.filter(c => c.result === 'WARNING');
      
      lines.push(`**Durchgeführt:** ${data.plausibilityChecks.length} Prüfungen`);
      ...
      if (failed.length > 0) {
        ...
        for (const check of failed) {
          lines.push(`- **${check.check_type}**: ${check.message || 'Keine Details'}`);
        }
        lines.push('');
      }
    ...
    const icOpenCount = data.icReconciliations.filter(ic => ic.status === 'open').length;
    const checkFailedCount = data.plausibilityChecks.filter(c => c.result === 'FAIL').length;
    
    if (icOpenCount === 0 && checkFailedCount === 0) {
      lines.push('[OK] **Keine wesentlichen Beanstandungen**');
      lines.push('');
      lines.push('Alle IC-Differenzen sind abgestimmt und alle Plausibilitätsprüfungen bestanden.');
    } else {
    ...
    lines.push('---');
    lines.push('');
    lines.push('*Dieses Dokument wurde automatisch generiert und dient als Entwurf für die Prüfdokumentation.*');
  ...
  private buildQualityIndicators(data: DocumentationData): QualityIndicators {
    ...
    if (data.consolidationEntries.length === 0) missingData.push('Konsolidierungsbuchungen');
    if (data.plausibilityChecks.length === 0) missingData.push('Plausibilitätsprüfungen');
    
    const completeness = 1 - (missingData.length / 2);
    const checksPass = data.plausibilityChecks.filter(c => c.result === 'PASS').length;
  ...
  private buildProvenance(data: DocumentationData): ProvenanceInfo[] {
    ...
    provenanceInfo.push(this.provenance.createDatabaseProvenance(
      'financial_statements',
      data.financialStatement.id,
      `Konzernabschluss ${data.financialStatement.fiscal_year}`,
    ));

    provenanceInfo.push(this.provenance.createHGBProvenance(
      '§ 290 HGB',
