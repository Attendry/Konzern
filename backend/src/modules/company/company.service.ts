...
export class CompanyService {
    ...
    try {
        ...
        (resolve) => {
          setTimeout(() => {
            resolve({
              data: null,
              error: {
                message: 'Supabase request timeout after 30 seconds',
                code: 'TIMEOUT',
              },
            });
          }, 30000);
      ...
      if (error) {
        ...
        if (error.code === 'TIMEOUT') {
          throw new Error(
            'Connection to Supabase timed out. Check your internet connection and Supabase configuration.',
          );
        }
      ...
      if (createCompanyDto.parentCompanyId && company.id) {
        try {
          const participationPercentage =
            createCompanyDto.participationPercentage ?? 100;
          this.logger.debug(
            `Creating participation: ${createCompanyDto.parentCompanyId} -> ${company.id} (${participationPercentage}%)`,
          );
          ...
          this.logger.warn(
            `Failed to create participation (non-critical): ${participationError.message}`,
          );
        }
      }
  ...
  async findAll(): Promise<Company[]> {
    ...
    try {
      ...
      this.logger.debug(
        `findAll() - Query completed in ${dbDuration}ms, found ${data?.length || 0} companies`,
      );

      if (error) {
        this.logger.error(
          `findAll() - Supabase error (${error.code}): ${error.message}`,
        );
      ...
      try {
        const companies = (data || []).map((item: any) => {
          try {
            ...
            this.logger.warn(
              `findAll() - Error mapping company ${item.id}: ${mapError.message}`,
            );
      ...
      this.logger.error(
        `findAll() - Error after ${totalDuration}ms: ${error.message}`,
      );
      if (
        error.message?.includes('timeout') ||
        error.message?.includes('timed out')
  ...
  async update(
    ...
    if (updateCompanyDto.taxId !== undefined)
      updateData.tax_id = updateCompanyDto.taxId;
    if (updateCompanyDto.address !== undefined)
      updateData.address = updateCompanyDto.address;
    if (updateCompanyDto.legalForm !== undefined)
      updateData.legal_form = updateCompanyDto.legalForm;
    if (updateCompanyDto.parentCompanyId !== undefined)
      updateData.parent_company_id = updateCompanyDto.parentCompanyId;
    if (updateCompanyDto.isConsolidated !== undefined)
      updateData.is_consolidated = updateCompanyDto.isConsolidated;
    ...
    if (updateCompanyDto.parentCompanyId !== undefined) {
      try {
        ...
        if (newParentId) {
          const participationPercentage =
            updateCompanyDto.participationPercentage ?? 100;
          this.logger.debug(
            `Updating participation: ${newParentId} -> ${company.id} (${participationPercentage}%)`,
          );
          await this.participationService.createOrUpdate({
        ...
        this.logger.warn(
          `Failed to update participation (non-critical): ${participationError.message}`,
        );
      }
    }
