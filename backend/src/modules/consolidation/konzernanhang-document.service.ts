...
import {
  ...
  ConsolidatedNotes,
} from './consolidated-notes.service';
...
export class KonzernanhangDocumentService {
  ...
  async createDocument(dto: CreateDocumentDto): Promise<KonzernanhangDocument> {
    ...
    if (fsError || !fs) {
      throw new NotFoundException(
        `Financial Statement mit ID ${dto.financialStatementId} nicht gefunden`,
      );
    }
  ...
  async updateSection(
    ...
    if (dto.sectionTitle !== undefined)
      updateData.section_title = dto.sectionTitle;
    if (dto.contentText !== undefined)
      updateData.content_text = dto.contentText;
    if (dto.contentJson !== undefined)
      updateData.content_json = dto.contentJson;
    if (dto.contentHtml !== undefined)
      updateData.content_html = dto.contentHtml;
    if (dto.manualOverride !== undefined)
      updateData.manual_override = dto.manualOverride;
  ...
  async generateAllSections(
    ...
    if (notes.significantEvents.length > 0) {
      const eventsSection = await this.createSection({
        ...
        contentText: notes.significantEvents
          .map((e, i) => `${i + 1}. ${e}`)
          .join('\n'),
  ...
  private generateGoodwillText(notes: ConsolidatedNotes): string {
    ...
    let text =
      'Die im Rahmen der Erstkonsolidierung entstandenen Unterschiedsbeträge ';
  ...
  private generateMinorityInterestsText(notes: ConsolidatedNotes): string {
    ...
    let text =
      'Die Anteile anderer Gesellschafter am Eigenkapital und am Jahresergebnis ';
  ...
  private generateIntercompanyText(notes: ConsolidatedNotes): string {
    ...
    let text =
      'Im Rahmen der Konsolidierung wurden folgende Zwischengesellschaftsgeschäfte eliminiert:\n\n';
  ...
  private generateRelatedPartiesText(notes: ConsolidatedNotes): string {
    ...
    let text =
      'Folgende Geschäfte wurden mit nahestehenden Unternehmen und Personen durchgeführt:\n\n';
  ...
  private async updateSectionCounts(documentId: string): Promise<void> {
    ...
    const completed = (data || []).filter(
      (s: any) => s.status === 'reviewed' || s.status === 'finalized',
    ).length;
