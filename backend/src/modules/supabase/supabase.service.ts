...
export class SupabaseService implements OnModuleInit {
  ...
  onModuleInit() {
    try {
      ...
      if (!supabaseUrl) {
        const projectRef = this.configService.get<string>(
      ...
      this.logger.log(
        `Supabase URL: ${supabaseUrl ? 'configured' : 'NOT SET'}`,
      );
      this.logger.log(
        `Supabase Key: ${supabaseKey ? 'configured' : 'NOT SET'}`,
      );

      if (!supabaseKey) {
        this.logger.warn(
          'Missing Supabase secret key. Set Supabase_Secret or SUPABASE_SECRET_KEY in .env.local',
        );
        this.logger.warn(
      ...
      if (!supabaseUrl) {
        this.logger.warn(
          'Missing SUPABASE_URL. Set SUPABASE_URL in .env.local',
        );
        this.logger.warn(
          'The backend will start, but database operations will fail.',
        );
    ...
    try {
      ...
      if (error) {
        if (
          ...
          this.logger.warn(
            'The "companies" table does not exist. Please run migrations.',
          );
        } else {
